...
def chat():
    ...
    return f"""
    <div class="user"> {user_input}</div></div>
    <div class="assistant"> {response_html}</div>
    """
...
def upload():
    ...
    user_html = f"<div class='user'>{user_input}</div>" if user_input else ""
    file_html = f"<div class='user'>{filename}</div>" if filename else ""
